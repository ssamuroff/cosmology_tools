...
gsl_spline * get_nchi_spline(c_datablock * block, int bin, double *z,
	gsl_spline * a_of_chi, gsl_spline * chi_of_z, shear_spectrum_config * config)
{
	int status = 0;
	double * N;
	int n;
	...
	status |= c_datablock_get_double_array_1d(block, config->LSS_survey, name, &N, &n);
...
gsl_spline * get_w_spline(c_datablock * block, int bin, double * z,
	double chi_max, gsl_spline * a_of_chi_spline, shear_spectrum_config * config)
{
	char name[20];
	double * n_of_z;
	int nz1;
	...
	// Load n(z)
	snprintf(name, 20, "bin_%d", bin);
	status |= c_datablock_get_double_array_1d(block, config->shear_survey, name, &n_of_z, &nz1);
...
int execute(c_datablock * block, void * config_in)
{
	...
	for (int bin=1; bin<=nbin; bin++){
		W_splines[bin-1] = get_w_spline(block, bin, z, chi_max, a_of_chi_spline, config);
		Nchi_splines[bin-1] = get_nchi_spline(block, bin, z, a_of_chi_spline, chi_of_z_spline, config);
