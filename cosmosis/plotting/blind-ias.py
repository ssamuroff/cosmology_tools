...
class plot2D(plots.MetropolisHastingsPlots2D):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.colors=['firebrick',"slateblue","purple", "slateblue", "forestgreen", "pink"]
        self.linestyles=['-','--',':','-']
        self.labels= [] #[ "Early", "Late", "Line 4"]
        if blind:
            blindx = np.random.rand()*0.2-0.1
            blindy =np.random.rand()*0.2-0.1
        else:
            blindx,blindy = 0,0
        x0 = 0.13* (1 + blindx)
        y0 = -2 * (1 + blindy)
        self.axis=[x0,x0+0.35, y0,y0+9] #[0.233,0.45,0.65,1.1]
        ...
        #self.proxies=[]
        self.axis_labels = {
                   ("cosmological_parameters", "omega_m"): "$\Omega_\mathrm{m}$",
                   ("cosmological_parameters", "sigma_8"): "$\sigma_8$",
                   ("cosmological_parameters", "n_s")    : "$n_\mathrm{s}$",
                   ("cosmological_parameters", "s8")     : "$S_8 \equiv \sigma_8 \left ( \Omega_\mathrm{m} / 0.3 \right )^{\frac{1}{2}}$",
                   ("wl_photoz_errors", "bias_1")        : "$\Delta z ^{(1)}$",
                   ("wl_photoz_errors", "bias_2")        : "$\Delta z ^{(2)}$",
                   ("wl_photoz_errors", "bias_3")        : "$\Delta z ^{(3)}$",
                   ("wl_photoz_errors", "bias_4")        : "$\Delta z ^{(4)}$",
                   ("intrinsic_alignment_parameters","a"): "$A_\mathrm{IA}$"}

    def run(self):
        name2="intrinsic_alignment_parameters--a"
    ...
    def make_2d_plot(self, name1, name2):
        ...
        try: pylab.xlabel(self.axis_labels[tuple(name1.split("--"))], fontsize=28)
        except: import pdb ; pdb.set_trace()
        pylab.ylabel(self.axis_labels[tuple(name2.split("--"))], fontsize=28)
        pylab.tight_layout()
        if blind:
