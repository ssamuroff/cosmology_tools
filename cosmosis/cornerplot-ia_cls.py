...
def get_theory_spectra(i,j,filename, cltype='GG'):
    if len(cltype)>0:
        cltype="_"+cltype
    ell = np.loadtxt("%s/shear_cl%s/ell.txt"%(filename,cltype.lower()))
    cl = np.loadtxt("%s/shear_cl%s/bin_%d_%d.txt"%(filename,cltype.lower(),i+1,j+1))
    return ell, cl
...
def cornerplot(theory1, theory2, data1, data2, show_cuts=False):
    ...
    for i in xrange(ni):
        for j in xrange(nj):
            ...
            ax.annotate("(%d, %d)"%(i+1,j+1), (0.4,-0.3), textcoords='data', fontsize=11, )
            ax.yaxis.set_tick_params(which='minor', left='off', right='off')
            plt.ylim(-0.5,1)
            #plt.yscale("log")
            plt.xscale("log")
            if posp==19:
                plt.ylabel(r"$\Delta C^{ij}_\mathrm{GI}(\ell)/C^{ij}_\mathrm{GG}(\ell)$", fontsize=12)
            ...
            xta,Cl_theory_a_ref = get_theory_spectra(i,j,theory1[0], cltype='gg')
            xtb,Cl_theory_b_ref = get_theory_spectra(i,j,theory2[0], cltype='gg')
            xta,Cl_theory_a_ref_ia = get_theory_spectra(i,j,theory1[0], cltype='gi')
            xtb,Cl_theory_b_ref_ia = get_theory_spectra(i,j,theory2[0], cltype='gi')
            ...
            for iline,(t1, t2) in enumerate(zip(theory1,theory2)):
                ...
                xta,Cl_theory_a = get_theory_spectra(i,j,t1, cltype='gi')
                xtb,Cl_theory_b = get_theory_spectra(i,j,t2, cltype='gi')

                plt.plot(xta, (Cl_theory_a-Cl_theory_a_ref_ia)/Cl_theory_a_ref, ls=linestyles[iline], color="red")
                plt.plot(xtb, (Cl_theory_b-Cl_theory_b_ref_ia)/Cl_theory_b_ref, ls=linestyles[iline], color="royalblue")

            posm = positions[(i+1,j+1,"-")]
            ax = plt.subplot(rows,cols,posm)
            ax.annotate("(%d, %d)"%(i+1,j+1), (0.4,0.35), textcoords='data', fontsize=11, )
            ax.yaxis.set_tick_params(which='minor', left='off', right='off')
            plt.ylim(-0.6,0.6)
            if posm==30:
                ...
                plt.yticks(fontsize=12)
                plt.ylabel(r"$\Delta C^{ij}_\mathrm{II}(\ell)/C^{ij}_\mathrm{GG}(\ell)$", fontsize=12)
            else:
                plt.yticks(visible=False)
            #plt.yscale("log")
            plt.xscale("log")
            ...
            xta,Cl_theory_a_ref = get_theory_spectra(i,j,theory1[0], cltype='gg')
            xtb,Cl_theory_b_ref = get_theory_spectra(i,j,theory2[0], cltype='gg')
            xta,Cl_theory_a_ref_ia = get_theory_spectra(i,j,theory1[0], cltype='ii')
            xtb,Cl_theory_b_ref_ia = get_theory_spectra(i,j,theory2[0], cltype='ii')
            ...
            for iline,(t1, t2) in enumerate(zip(theory1,theory2)):
                ...
                xta,Cl_theory_a = get_theory_spectra(i,j,t1, cltype='ii')
                xtb,Cl_theory_b = get_theory_spectra(i,j,t2, cltype='ii')
