...
class Measure2Point(PipelineStage):
    ...
    def call_treecorr(self,i,j,k):
        ...
        if (k==0): # xi+-
            theta,xip,xim,xiperr,ximerr = self.calc_shear_shear(i,j,verbose,num_threads)
        else:
            xip=xim=xiperr=ximerr=None

        if (k==1): # gammat
            theta,gammat,gammaterr = self.calc_pos_shear(i,j,verbose,num_threads)
        else:
    ...
    def write(self):
        ...
        for n,name in enumerate(TWO_POINT_NAMES):
            ...
            f = None 
            for (theta,xi_data,ijk) in zip(self.theta, self.xi, self.calc):
                i,j,k = ijk
